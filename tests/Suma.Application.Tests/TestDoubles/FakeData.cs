...
internal sealed class FakeData : IAccountStore, ICategoryStore, ITransactionStore, IBudgetStore, IBudgetAllocationStore, IRecurringTransactionStore, IRecurringOccurrenceStore, ISavingsGoalStore, IGoalContributionStore, IOverviewStore, IReportStore, IUnitOfWork
{
    ...
    Task ITransactionStore.RemoveAsync(Transaction transaction, CancellationToken cancellationToken)
    {
        Transactions.Remove(transaction.Id);
        return Task.CompletedTask;
    }
    Task<bool> ITransactionStore.HasRefundsAsync(Guid transactionId, CancellationToken cancellationToken) =>
        Task.FromResult(RefundedAmountMinor > 0 || Transactions.Values.Any(item => item.Type == TransactionType.Refund && item.OriginalTransactionId == transactionId));
    ...
    Task<RecurringOccurrence?> IRecurringOccurrenceStore.GetByTransactionIdAsync(Guid transactionId, CancellationToken cancellationToken) =>
        Task.FromResult(Occurrences.Values.FirstOrDefault(item => item.TransactionId == transactionId));
